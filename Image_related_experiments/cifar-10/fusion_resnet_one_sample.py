...
def get_topk_pixels(attr_map, drop_ratio=0.5):
    ...
    # Sort by the absolute value of significance from large to small, select top k (most important pixels)
...
def drop_features(img_tensor, drop_coords, drop_value=0):
    """Discard the feature at the specified coordinates (set the area to be discarded as drop_value)"""
...
def visualize_drop_results(img_original, dropped_imgs, method_names, drop_ratio, device, save_path='drop_results_all.png'):
    ...
    for i, (img_dropped, method) in enumerate(zip(dropped_imgs, method_names)):
        ...
        ax.set_title(f'{method}移除{drop_ratio*100}%特征')
        ax.axis('off')
...
def visualize_final_attr_map(img_original, final_attr_map, inconsistent_methods, save_path='final_attr_map.png'):
    """
    生成最终的特征重要性热力图
    :param img_original: 原始图片(PIL格式)
    :param final_attr_map: 融合后的最终特征重要性图谱
    :param inconsistent_methods: 预测不一致的解释器列表
    ...
    # 显示最终热力图
    ax2.set_title(f'Final Feature Importance\nInconsistent Interpreter(s): {", ".join(inconsistent_methods)}' 
                  if inconsistent_methods else 'Final Feature Importance (All Consistent)')
    im = ax2.imshow(final_attr_map, cmap='jet')
    ax2.axis('off')
...
def save_single_sample_result(img_idx, true_class, true_label, exp_result, drop_ratio, save_dir):
    ...
    with open(result_path, 'w', encoding='utf-8') as f:
        ...
        f.write("="*50 + "\n")
        # 记录不一致的解释器
        f.write(f"Inconsistent methods: {', '.join(exp_result['inconsistent_methods'])}\n")
        f.write("="*50 + "\n")
        # 记录各基础解释器结果
        for method in exp_result['base_method_names']:
            ...
            f.write("\n")
        # 新增：记录最终融合解释器结果
        final_res = exp_result['method_results']['Fusion']
        f.write("Fusion：\n")
        f.write(f"  dropped_class: {final_res['dropped_class']}\n")
        f.write(f"  consistency: {final_res['consistency']}\n")
        f.write(f"  based_on_methods: {', '.join(exp_result['inconsistent_methods']) if exp_result['inconsistent_methods'] else 'All Methods'}\n")
...
def get_attribute_maps(model, img_tensor, device):
    ...
    shap_attr_tensor = torch.tensor(shap_attr_4x4).unsqueeze(0).unsqueeze(0).float()
...
def drop_experiment_all_methods(model, img_tensor, device, drop_ratio=0.5):
    ...
    base_method_names = list(attr_maps.keys())  # ['LIME', 'SHAP', 'Grad-CAM', 'IG']
    ...
    # 4. 收集不一致解释器并生成最终特征重要性图谱
    inconsistent_methods = [m for m in base_method_names if method_results[m]['consistency'] == 0]
    inconsistent_attr_maps = [method_results[m]['attr_map'] for m in inconsistent_methods]
    
    if len(inconsistent_attr_maps) > 0:
        final_attr_map = np.mean(inconsistent_attr_maps, axis=0)  # 融合不一致解释器的特征重要性
    else:
        final_attr_map = np.mean([method_results[m]['attr_map'] for m in base_method_names], axis=0)  # 所有解释器一致时，融合全部
    
    final_attr_map = (final_attr_map - final_attr_map.min()) / (final_attr_map.max() - final_attr_map.min() + 1e-8)  # 归一化
    
    # 5. 基于最终特征重要性的特征删除与一致性计算
    final_drop_coords = get_topk_pixels(final_attr_map, drop_ratio=drop_ratio)
    final_img_dropped = drop_features(img_tensor, final_drop_coords)
    with torch.no_grad():
        final_dropped_output = model(final_img_dropped.unsqueeze(0).to(device))
        final_dropped_pred = final_dropped_output.argmax(dim=1).item()
        final_dropped_class = class_names[final_dropped_pred]
    final_consistency = 1 if orig_pred == final_dropped_pred else 0
    
    # 6. 记录最终融合解释器的结果
    method_results['Fusion'] = {
        'dropped_class': final_dropped_class,
        'consistency': final_consistency,
        'attr_map': final_attr_map,
        'dropped_img': final_img_dropped
    }
    
    # 7. 整理输出结果
    all_method_names = base_method_names + ['Fusion']
    all_dropped_imgs = dropped_imgs + [final_img_dropped]
    all_attr_maps_list = [method_results[m]['attr_map'] for m in all_method_names]
    
    return {
        'orig_class': orig_class,
        'orig_pred': orig_pred,
        'method_results': method_results,
        'base_method_names': base_method_names,  # 基础解释器列表
        'all_method_names': all_method_names,    # 所有解释器（含最终融合）
        'dropped_imgs': all_dropped_imgs,        # 所有解释器删除后的图片
        'all_attr_maps_list': all_attr_maps_list,  # 所有解释器的特征重要性图谱
...
if __name__ == "__main__":
    ...
    result_dir = 'fusion/resnet18/result_0.5_with_fusion_4962_v4'
    ...
    DROP_RATIO = 0.5
    NUM_SAMPLES = 10000
    TARGET_SAMPLE_INDEX = 4962
    RANDOM_SEED = 42
    MAX_SAMPLES = 10  # 限制测试样本数量（可调整）
    ...
    for epoch in range(EPOCHS):
        ...
        test_loss, test_acc = test(model, test_loader_full, criterion, device)
        scheduler.step()
    ...
    batch_results = {
        ...
        'IG': {'consistencies': []},
        'Fusion': {'consistencies': []}
    }

    total_exp_samples = 0
    correct_exp_samples = 0  # 模型预测正确的样本数
    
    #for img_idx, (img_tensor, true_label) in enumerate(tqdm(test_loader, desc="experimental progress")):
        #if img_idx >= MAX_SAMPLES:
        #    break

    img_tensor, true_label_single = test_dataset[TARGET_SAMPLE_INDEX]
    true_class = class_names[true_label_single]
    img_tensor = img_tensor.to(device)  # 移至指定设备
    print(f"\n=== 开始测试4962号样本 ===")
    print(f"样本索引: {TARGET_SAMPLE_INDEX}")
    print(f"真实标签: {true_label_single} ({true_class})")
    ...
    # 可视化所有解释器的特征重要性图谱（含Fusion）
    img_original = denormalize(img_tensor, device)
    visualize_attr_maps(
        img_original,
        exp_result['all_attr_maps_list'],
        exp_result['all_method_names'],
        save_path=f'{result_dir}/sample_{TARGET_SAMPLE_INDEX:04d}_all_attr_maps.png'
    )
    
    # 可视化所有解释器删除后的图片（含Fusion）
    visualize_drop_results(
        ...
        DROP_RATIO,
        device,
        save_path=f'{result_dir}/sample_{TARGET_SAMPLE_INDEX:04d}_all_drop_results.png'
    )
    
    # 可视化最终特征重要性热力图
    visualize_final_attr_map(
        img_original,
        exp_result['final_attr_map'],
        exp_result['inconsistent_methods'],
        save_path=f'{result_dir}/sample_{TARGET_SAMPLE_INDEX:04d}_final_attr_map.png'
    )
    ...
    # 计算各解释器平均一致性
    avg_consistencies = {}
    all_method_names = list(batch_results.keys())
    ...
    print(f"Number of test samples: {total_exp_samples}")
    ...
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']  # 新增紫色用于Fusion
    methods_display = ['LIME', 'SHAP', 'Grad-CAM', 'Integrated Gradients', 'Fusion']
    
    bars = plt.bar(
        methods_display,
        [avg_consistencies[method] for method in all_method_names],
        color=colors,
        alpha=0.8
    )
    ...
    # 在柱状图上添加数值标签
    for bar, val in zip(bars, [avg_consistencies[method] for method in all_method_names]):
        plt.text(
    ...
    plt.tight_layout()
    plt.savefig(f'{result_dir}/all_methods_consistency_comparison.png', dpi=600, bbox_inches='tight')
    plt.close()
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"\nFusion explanation:\n")
        f.write(f"  - If there are inconsistent base interpreters, fuse their feature importance;\n")
        f.write(f"  - If all base interpreters are consistent, fuse all base interpreters' feature importance.\n")
